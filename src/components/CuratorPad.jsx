...
export default function CuratorPad() {
  const [inputText, setInputText] = useState(`# DevOps Lab Notes Workspace

Paste your markdown code blocks or instructions here to read them under a custom view.

- **Fast builds**: Docker caching handles intermediate layers.
- **Portability**: Write once, deploy anywhere.

\`\`\`bash
# Run a container in the background
docker run -d -p 80:80 --name web-nginx nginx:alpine
\`\`\``);
