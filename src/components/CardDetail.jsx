...
export default function CardDetail() {
  ...
  // Load experiment data with local caching
  useEffect(() => {
    const cachedData = localStorage.getItem(`devops_exp_${id}`);
    const found = experimentsData.find((item) => item.id === id);
    
    // Simulate premium loader skeleton for AI generation
    const timer = setTimeout(() => {
      if (found) {
        let currentExperiment = found;
        if (cachedData) {
          const parsed = JSON.parse(cachedData);
          // Sync cache if the default json data has changed
          if (parsed.question !== found.question || parsed.commands !== found.commands || parsed.title !== found.title || JSON.stringify(parsed.options) !== JSON.stringify(found.options)) {
            localStorage.setItem(`devops_exp_${id}`, JSON.stringify(found));
            currentExperiment = found;
          } else {
            currentExperiment = parsed;
          }
        } else {
          localStorage.setItem(`devops_exp_${id}`, JSON.stringify(found));
        }
  ...
  // Extract and copy all code blocks
  const copyAllCommands = () => {
    const textToParse = currentCommands;
    const matches = [...textToParse.matchAll(/```(?:\w*)\n([\s\S]*?)```/g)];
    const allCmds = matches.map(m => m[1].trim()).join('\n\n');

    if (allCmds) {
      navigator.clipboard.writeText(allCmds)
        .then(() => showToast('All Commands Copied Successfully'))
        .catch(err => console.error('Failed to copy all commands:', err));
    } else {
      showToast('No commands found to copy');
    }
  };

  // Compile full experiment document into Markdown and copy
  const copyCompleteExperiment = () => {
    const optionSuffix = hasOptions ? ` (${experiment.options[activeOptionIndex].name})` : '';
    const fullMarkdown = `# Experiment ${experiment.number}: ${experiment.title}${optionSuffix}
...
${currentCommands}`;
    ...
    navigator.clipboard.writeText(fullMarkdown)
      .then(() => showToast('Question Copied Successfully'))
      .catch(err => console.error('Failed to copy experiment:', err));
  };
    ...
    <div className="fade-in" style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '32px', textAlign: 'left' }}>
      ...
      <div className="detail-header-controls">
        ...
        <div className="detail-copy-actions">
          <button onClick={copyAllCommands} className="glass-btn">
            Copy All Commands
          </button>
          <button onClick={copyCompleteExperiment} className="glass-btn glass-btn-primary">
            Copy Complete Experiment
          </button>
        </div>
      </div>
      ...
      <div style={{ paddingBottom: '10px' }}>
        <span style={{ fontSize: '14px', fontWeight: '600', color: 'var(--accent-primary)', textTransform: 'uppercase', letterSpacing: '1.5px' }}>
          DevOps Lab Manual · Exercise {experiment.number}
        </span>
      ...
      <div style={{ 
        ...
        borderRight: '1px solid rgba(79, 70, 229, 0.05)',
